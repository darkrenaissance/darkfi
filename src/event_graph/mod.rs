...
impl EventGraph {
    ...
    pub async fn new(
        ...
        let register_zkbin = ZkBinary::decode(RLN2_REGISTER_ZKBIN).unwrap();
        ...
        // Read or build signal verifying key
        let signal_zkbin = ZkBinary::decode(RLN2_SIGNAL_ZKBIN).unwrap();
        ...
        let slash_zkbin = ZkBinary::decode(RLN2_SLASH_ZKBIN).unwrap();
    ...
    pub async fn dag_sync(&self, dag: sled::Tree, fast_mode: bool) -> Result<()> {
        ...
        let comms_timeout = self.p2p.settings().read().await.outbound_connect_timeout;
    ...
    pub async fn dag_insert(&self, events: &[Event], dag_name: &str) -> Result<Vec<Hash>> {
        ...
        for event in events {
            ...
            debug!(
                ...
                "Inserting event {} into the DAG layer: {}", event_id, event.header.layer,
            );
        ...
        for event in events {
            ...
            debug!(
                target: "event_graph::dag_insert()",
                "Event {} parents {:#?}", event_id, event.header.parents,
            );
