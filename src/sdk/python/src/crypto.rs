...
pub(crate) fn create_module(py: Python<'_>) -> PyResult<Bound<PyModule>> {
    ...
    submod.clone().add_function(wrap_pyfunction!(poseidon_hash, submod.clone())?)?;
    submod.clone().add_function(wrap_pyfunction!(pedersen_commitment_u64, submod.clone())?)?;
    submod.clone().add_function(wrap_pyfunction!(pedersen_commitment_base, submod.clone())?)?;
    Ok(submod)
}
