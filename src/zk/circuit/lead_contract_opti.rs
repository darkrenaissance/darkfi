...
const LEADCOIN_PK_X_OFFSET: usize = 0;
/// Public input offset for lead coin public key Y coordinate
const LEADCOIN_PK_Y_OFFSET: usize = 1;
/// Public input offset for the lead coin C2 nonce
const LEADCOIN_C2_NONCE_OFFSET: usize = 2;
...
pub struct LeaderCircuit {
    ...
    pub coin1_commitment_root: Value<pallas::Base>,
    /// `coin_1` leaf position in the Merkle tree of coin commitments
    pub coin1_leaf_pos: Value<u32>,
    /// `coin_1` secret key.
    ...
    /// eta is calculated as a random value using some parameters.
    /// It is defined as true randomness from global random clock.
    pub eta: Value<pallas::Base>,
...
impl Circuit<pallas::Base> for LeaderCircuit {
    ...
    fn synthesize(
        ...
        let coin1_commitment_root = assign_free_advice(
            layouter.namespace(|| "witness coin_commitment_root"),
            config.advices[8],
            self.coin1_commitment_root,
        )?;
        ...
        let eta = assign_free_advice(
            layouter.namespace(|| "witness eta randomness"),
            config.advices[8],
            self.eta,
        )?;
        ...
        // Constrain equality of derived root with witnessed root
        layouter.assign_region(
            || "coin1_cm_root equality",
            |mut region| region.constrain_equal(coin1_cm_root.cell(), coin1_commitment_root.cell()),
        )?;
        ...
        let (rho_commit_r, _) = {
            let r = FixedPoint::from_inner(ecc_chip.clone(), ValueCommitR);
            r.mul(layouter.namespace(|| "mau_rho * ValueCommitR"), mau_rho)?
        };

        let rho_commit =
            y_commit.add(layouter.namespace(|| "y_commit + rho_commit_r"), &rho_commit_r)?;
        ...
        // At this point we've enforced all of our constraints
        Ok(())
    }
}
...
mod tests {
    ...
    fn test_leader_circuit() -> Result<()> {
        let k = 11;
        let circuit = LeaderCircuit::default();

        let root = BitMapBackend::new("target/leader_circuit_layout.png", (3840, 2160))
            .into_drawing_area();
        root.fill(&WHITE).unwrap();
        let root = root.titled("Leader Circuit Layout", ("sans-serif", 60)).unwrap();
